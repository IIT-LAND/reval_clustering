...
glass = {'data': np.array([vect[1:-1] for vect in data_dict['glass']]).astype(float),
         ...
         'description': "From USA Forensic Science Service; 6 types of glass; defined in terms of their "
                        ...
                        "Type of glass: 0 building_windows_float_processed; 1 building_windows_non_float_processed; "
...
# Parkinson's
parkinsons = {
    'data': np.array([vect[1:len(vect) - 7] + vect[len(vect) - 6::] for vect in data_dict['parkinsons']]).astype(float),
...
seeds = {'data': np.array([vect[:-1] for vect in data_dict['seeds_dataset']]).astype(float),
         ...
         'description': "Measurements of geometrical properties of kernels belonging to three different varieties of "
                        "wheat (0-2). "
...
transfusion = {'data': np.array([vect[:-1] for vect in data_dict['transfusion']]).astype(float),
               ...
               'description': "To demonstrate the RFMTC marketing model (a modified version of RFM), "
                              ...
                              "in Hsin-Chu City in Taiwan. The center passes their blood transfusion service bus to "
...
leaf = {'data': np.array([vect[2:-1] for vect in data_dict['leaf']]).astype(float),
        'target': np.array([int(vect[0]) - 1 for vect in data_dict['leaf']]).astype(int),
        'description': "This dataset consists in a collection of shape and texture features extracted from "
                       "digital images of leaf specimens originating from a total of 36 (0-35) "
...
forest = {'data': np.array(
    [vect[1::] for vect in data_dict['forest_tr']] + [vect[1::] for vect in data_dict['forest_ts']]).astype(float),
          ...
          'description': "Multi-temporal remote sensing data of a forested area in Japan. It includes both training "
                         "and test datasets. "
...
with open('./datasets/uci_datasets.pkl', 'wb') as file:
    pkl.dump(data_collection, file)
